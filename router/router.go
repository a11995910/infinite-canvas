...
func New() *gin.Engine {
	...
	api.POST("/auth/sub2api-embed", gin.WrapF(handler.Sub2APIEmbedLogin))
	api.GET("/sub2api/keys", gin.WrapF(handler.Sub2APIEmbedKeys))
	api.POST("/sub2api/session", gin.WrapF(handler.Sub2APIEmbedSession))
	api.Any("/sub2api/proxy/:target/:expires/:signature/*path", handler.Sub2APIProxy)
