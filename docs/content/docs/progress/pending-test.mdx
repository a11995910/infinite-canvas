...
- Vite 与 Go API 联动：本地启动 Go 服务和 Vite 后，登录、画布同步、服务端文件接口及 `/api/health` 均应通过 Vite 代理可用；Docker 容器中 Nginx 的 `/api/*` 反代应保持相同行为。
- Linux.do 登录：第三方回调返回 `/login?token=...` 后，Vite 登录页应读取令牌、获取当前用户并跳转回原页面。
- Sub2API iframe 嵌入：允许来源、有效登录令牌和有效 Key 下应自动创建画布会话并按图片、文本、视频填充渠道；来源不在白名单、令牌失效、签名过期和内网来源未显式允许时应给出对应失败提示。
- Sub2API Grok 视频：Grok 分组且具备图片生成权限的 Key 应自动配置 `grok-imagine-video-1.5` 视频模型；生成请求应使用 `/v1/videos/generations`，随后按 `request_id` 查询 `/v1/videos/{request_id}` 并保存 `video.url`。视频设置应显示 480p、720p、1080p 和 1-15 秒，1080p 仅在 `grok-imagine-video-1.5` 且添加 1 张首帧图时可提交。
- OpenAI 兼容渠道：Base URL 支持填写站点根地址、以 `/v1` 结尾的 OpenAI Base URL、`llms.txt` / `llms-full.txt` 文档地址，以及误填的 `/v1/models`、`/v1/images/generations`、`/v1/images/edits` 等完整接口地址；需使用 `https://api.skylee9.cloudns.ch`、`https://api.skylee9.cloudns.ch/v1` 和 `https://api.skylee9.cloudns.ch/llms.txt` 分别拉取模型，并用 `gpt-image-2` 验证文生图和图生图均请求到正确的 `/v1` JSON 接口。
