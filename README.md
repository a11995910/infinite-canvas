...
- 账号与嵌入：保留账号登录、画布同步和 Sub2API iframe 嵌入；嵌入时会校验 Sub2API 登录态并自动配置当前账号的可用 Key。
...
AI API Key、Base URL、素材和生成记录默认保存在浏览器本地。画布项目可在登录后由服务端同步，Sub2API 嵌入、登录和同步接口由同源 Go API 提供。
...
运行后访问 `http://localhost:13000`。生产环境可使用默认 Compose 配置，它只绑定 `127.0.0.1:13000`，适合交由反向代理提供 HTTPS。
