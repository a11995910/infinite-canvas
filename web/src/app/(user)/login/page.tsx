...
function LoginContent() {
    ...
    }, [message, redirect, router, searchParams, setSession, sub2apiEmbedded]);

    useEffect(() => {
        if (sub2apiEmbedded && user) router.replace(withSub2APIEmbedParams("/canvas"));
    }, [router, sub2apiEmbedded, user]);
        ...
        return (
            <main className="flex h-full min-h-0 items-center justify-center overflow-y-auto bg-background px-6 py-10 text-stone-950 dark:text-stone-100">
                <section className="w-full max-w-md text-center">
                    <span
                        className="mx-auto mb-5 block size-12 bg-stone-950 dark:bg-stone-100"
                        style={{
                        ...
                        aria-label="无限画布"
                    />
                    <h1 className="text-2xl font-semibold tracking-normal">{isEmbedLoginFailed ? "Sub2API 登录状态无效" : "正在进入无限画布"}</h1>
