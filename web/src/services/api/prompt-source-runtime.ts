...
function extractImages(baseUrl: string, markdown: string) {
    ...
    // 上游 README 混用 HTML 和 Markdown，优先提取案例图并去除重复地址。
    for (const pattern of [/<img[^>]+src=["']([^"']+)["']/gi, /!\[[^\]]*]\(([^)]+)\)/g]) {
        for (const match of markdown.matchAll(pattern)) {
            const image = absoluteUrl(baseUrl, match[1]);
...
export const PROMPT_SOURCE_VARIABLES: PromptSourceVariable[] = [
    ...
    { name: "extractImages", type: "function", desc: "extractImages(baseUrl, markdown) 提取 HTML、Markdown 图片并补全为绝对地址" },
