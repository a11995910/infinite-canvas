...
export function resolveVideoModel(config: Pick<AiConfig, "model" | "videoModel">) {
    return config.model || config.videoModel;
}
        ...
        }),
        {
            ...
            merge: (persisted, current) => {
                ...
                const legacyModel = (model: string, channelId?: string) => {
                    const channel = channels.find((item) => item.id === channelId) || channels.find((item) => item.models.some((entry) => entry.name === modelOptionName(model))) || channels[0];
