...
+ [调整] Sub2API Seedance 视频请求同步统一 JSON 协议，可发送现有参考图片、视频、音频和原生音频开关，并按固定清晰度与 sd8 模型能力省略不支持字段。

+ [修复] 视频工作台与画布会持续恢复待确认任务，成功但无产物继续待确认，用户停止仅终止本地等待。

+ [调整] 视频生成区分明确失败与结果待确认，网络异常、查询超时和用户停止不再显示为可重试失败，并保留任务供后续查询。
+ [修复] Sub2API 图片生成和编辑成功后由服务端保存 Base64 图片，并向浏览器返回短期同源图片地址，避免 4K 图片响应在代理链路中解析失败。
+ [修复] Sub2API 嵌入模型按每把 Key 所属分组严格过滤，并隐藏本地默认渠道，避免按条 Key 混入按秒模型。
+ [新增] Sub2API 嵌入模式下的视频模型选择框显示当前用户实际结算价，并按计费方式标注灵石/条或灵石/秒。
+ [修复] Sub2API 嵌入代理地址改为同源相对路径，避免本地开发及反向代理环境中的跨域模型请求失败。
+ [修复] 所有视频模型的生成任务轮询等待上限统一延长至 20 分钟，单次查询异常最多连续尝试 3 次，并兼容更多终态与生成进度。
+ [修复] Sub2API 嵌入页会在渠道配置就绪后恢复视频任务，页面离开时停止本地轮询，并支持手动停止等待。
+ [优化] 生成视频按存储模式保存到服务端或浏览器本地，混合模式可回退本地；下载内容、媒体元数据和超大参考请求会明确校验。
+ [修复] 视频生成记录按实际更新时间同步，避免旧的“生成中”记录覆盖已完成结果。

## v0.9.2 - 2026-07-21

+ [新增] 画布生图支持从资源库多选图片作为可见参考节点，并将生图 API 等待上限延长至 10 分钟。

## v0.9.1 - 2026-07-20

+ [修复] Sub2API 嵌入渠道的 Seedance 视频改用 OpenAI `/v1/videos` 协议创建和轮询任务。
+ [修复] Sub2API 嵌入登录和渠道拉取会透传浏览器 User-Agent，避免上游会话指纹校验导致 401 和渠道消失。
+ [修复] 配置弹窗及各生成入口的模型下拉按渠道模型能力过滤，不再混入其他类型模型。
+ [调整] Sub2API iframe 嵌入会注入全部活跃的 OpenAI、Gemini 和 Grok Key，并按 Key 拉取模型供各类生成下拉切换。
+ [调整] Sub2API 代理签名默认有效期延长至 7 天，减少长时间打开嵌入页面后代理地址失效的情况。
+ [修复] 画布内层代理延长 AI 请求等待时间并支持 100MB 请求体，避免生成超时和参考图上传被拦截。
+ [修复] Grok 视频生成会在提交前自动压缩超限参考图，避免 Sub2API 的 1MB 内联上传限制。
+ [修复] 提示词库恢复解析上游 HTML 图片和相对图片路径，避免大量案例显示为空封面。
